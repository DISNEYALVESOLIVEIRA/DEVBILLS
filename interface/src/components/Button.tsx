...
const Button = ({
	...
	return (
		<button
			type="button"
			className={`cursor-pointer px-5 py-2.5 rounded-xl font-medium transition-all flex items-center justify-center gap-2 ${variantClasses[variant]} ${isLoading || disabled ? "opacity-50 cursor-not-allowed" : ""} ${className ?? ""} ${fullWidth ? "w-full" : ""}`}
